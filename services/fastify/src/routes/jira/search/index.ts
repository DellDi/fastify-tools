  ...
  fastify.post('', {
    ...
    handler: async (req, reply) => {
      try {
        ...
        // 记录请求信息（不包含敏感信息）
        fastify.log.info(`Jira 搜索请求: startAt=${startAt}, maxResults=${maxResults}`)
        fastify.log.info(`JQL: ${jql}`)
        ...
        try {
          ...
          if (jiraRes?.statusCode !== 200) {
            ...
            fastify.log.error(`Jira API 错误: ${jiraRes.statusCode}, ${errorBody}`)
          ...
          fastify.log.info(`Jira 搜索成功，返回 ${jiraData.issues?.length || 0} 条记录`)
