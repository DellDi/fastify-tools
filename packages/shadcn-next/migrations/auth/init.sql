...
VALUES ('密码解析', '#', 'SquareTerminal', NULL, NULL), -- 一级菜单，不需要权限控制
       ('v10加密中心', '/password/newsee', 'History', 'neesee密码加解密',
        (SELECT id FROM auth.menus WHERE name = '密码解析')),

       ('文件系统', '#', 'Bot', NULL, NULL),            -- 一级菜单
       ('文件管理', '/file/manage', 'Rabbit', '我们最快的通用模型。',
        (SELECT id FROM auth.menus WHERE name = '文件系统')),
       ('文件上传', '/file/upload', 'Bird', '高效的性能和速度。', (SELECT id FROM auth.menus WHERE name = '文件系统')),
       ('大文件上传', '/file/bigLoad', 'Turtle', '最强大的复杂计算模型。',
        (SELECT id FROM auth.menus WHERE name = '文件系统')),

       ('jira中心', '#', 'BookOpen', NULL, NULL),       -- 一级菜单
       ('个人看板', '/jira/personal', NULL, NULL, (SELECT id FROM auth.menus WHERE name = 'jira中心')),
       ('创建工单', '/jira/create', NULL, NULL, (SELECT id FROM auth.menus WHERE name = 'jira中心')),

       ('AIGC', '#', 'Code2', NULL, NULL),              -- 一级菜单
       ('Chat工具', '/aigc/chat', NULL, NULL, (SELECT id FROM auth.menus WHERE name = 'AIGC')),
       ('模型调优', '/aigc/model', NULL, NULL, (SELECT id FROM auth.menus WHERE name = 'AIGC')),

       ('设置', '#', 'Settings2', NULL, NULL),          -- 一级菜单
       ('账户', '/settings', NULL, NULL, (SELECT id FROM auth.menus WHERE name = '设置')),
       ('角色', '/role', NULL, NULL, (SELECT id FROM auth.menus WHERE name = '设置'));
...
INSERT INTO auth.role_menus (role_id, menu_id)
SELECT (SELECT id FROM auth.roles WHERE name = 'user'), id
FROM auth.menus
